...
export default function Wrap_other(props) {
  ...
  return (
    <div className="bg-fourth-light mt-10 h-screen p-10 flex flex-col items-center justify-center">
      <h1 className="text-4xl font-bold mb-2 font-itim  ">Meet My Friends</h1>
      <p className="text-xl mb-4 font-lusitana">
        Wanna hear what other people think about me?  </p>
      <div className="p-5 flex left-1/3 item-left w-full">
        <Card_other className={` w-[20rem] blur-sm scale-75 shadow-sm translate-x-[10em] `} index={left} name={leftData.name} image="joseph.jpg"  />
        <Card_other className={` w-[40rem] shadow-sm ${isClicked ? 'animate-left-lg' : 'z-10'} : ''}`} index={cur} name={curData.name} image="joseph.jpg" content={curData.content} title={curData.title}/>
        <Card_other className={` w-[20rem] shadow-sm blur-sm scale-75 translate-x-[-10em] z-1 ${isClicked ? 'animate-zoom-lg' : ''} `} index={right} name={rightData.name} image="joseph.jpg" />
        <Card_other className={` shadow-sm z-0 w-[20rem] absolute blur-sm scale-75 translate-x-[50em]`} index={right2} name={cur} image="joseph.jpg"/>
